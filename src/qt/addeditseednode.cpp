...
AddEditSeedNode::AddEditSeedNode(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::AddEditSeedNode)
{
    ui->setupUi(this);

}

AddEditSeedNode::~AddEditSeedNode()
{
    delete ui;
}


void AddEditSeedNode::on_okButton_clicked()
{
	...
	bool bForceNew = false;
        ...
        pwalletMain->mapMySeedNodes.insert(make_pair(c.sAddress, c));
	walletdb.WriteSeedNodeConfig(c.sAddress, c);
        uiInterface.NotifySeedNodeChanged(c);

        accept();
    }
}

void AddEditSeedNode::on_cancelButton_clicked()
{
    reject();
}
