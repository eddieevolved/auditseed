...
void HexAddressConverter::addressChanged(const QString& address) {
    ...
    if(!address.isEmpty()) {
        ...
        hasSeedAddress = IsValidDestination(addr);
        if (hasSeedAddress) {
            ui->label->setText("Address (Seed)");
        ...
        isAddressValid = hasHexAddress || hasSeedAddress;
    }
