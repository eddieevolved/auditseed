...
void setupAddressWidget(QValidatedLineEdit* widget, QWidget* parent)
{
    ...
    // and this is the only place, where this address is supplied.
     widget->setPlaceholderText(QObject::tr("Enter a Seed address (e.g. %1)").arg("LhfHsuK9Ekjj2dYoEy4wvEiQHvRtzZQnYa"));
#endif
...
{
    // return if URI is not valid or is no SEED: URI
...
{
    // Convert seed:// to seed:
    //
    //    Cannot handle this later, because seed:// will cause Qt to see the part after // as host,
...
void openConfigfile()
{
    ...
    /* Open seed.conf with the associated application */
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "SEED.lnk";
}
...
{
    return GetAutostartDir() / "seed.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    if (!fAutoStart)
        ...
        if (!optionFile.good())
            return false;
        // Write a seed.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        optionFile << "Name=SEED\n";
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the seed app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if (fAutoStart && !foundItem) {
        // add seed app to startup item list
