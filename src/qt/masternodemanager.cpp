...
static void NotifySeedNodeUpdated(MasternodeManager *page, CSeedNodeConfig nodeConfig)
{
    ...
    QMetaObject::invokeMethod(page, "updateSeedNode", Qt::QueuedConnection,
                              Q_ARG(QString, alias),
                              Q_ARG(QString, addr),
...
void MasternodeManager::subscribeToCoreSignals()
{
    ...
    uiInterface.NotifySeedNodeChanged.connect(boost::bind(&NotifySeedNodeUpdated, this, _1));
}

void MasternodeManager::unsubscribeFromCoreSignals()
{
    // Disconnect signals from core
    uiInterface.NotifySeedNodeChanged.disconnect(boost::bind(&NotifySeedNodeUpdated, this, _1));
}
...
void MasternodeManager::updateSeedNode(QString alias, QString addr, QString privkey, QString collateral)
{
    LOCK(cs_adrenaline);
    bool bFound = false;
    int nodeRow = 0;
...
void MasternodeManager::updateNodeList()
{
    ...
    {
        LOCK(cs_adrenaline);
        for (PAIRTYPE(std::string, CSeedNodeConfig) adrenaline : pwalletMain->mapMySeedNodes)
        {
            updateSeedNode(QString::fromStdString(adrenaline.second.sAlias), QString::fromStdString(adrenaline.second.sAddress), QString::fromStdString(adrenaline.second.sMasternodePrivKey), QString::fromStdString(adrenaline.second.sCollateralAddress));
        }
    }
}
...
{
    AddEditSeedNode* aenode = new AddEditSeedNode();
    aenode->exec();
}
#if 0
...
void MasternodeManager::on_getConfigButton_clicked()
{
    ...
    int r = index.row();
    std::string sAddress = ui->tableWidget_2->item(r, 1)->text().toStdString();
    CSeedNodeConfig c = pwalletMain->mapMySeedNodes[sAddress];
    std::string sPrivKey = c.sMasternodePrivKey;
    SeedNodeConfigDialog* d = new SeedNodeConfigDialog(this, QString::fromStdString(sAddress), QString::fromStdString(sPrivKey));
    d->exec();
}
...
void MasternodeManager::on_removeButton_clicked()
{
    ...
    if(confirm == QMessageBox::Yes)
    {
        ...
        pwalletMain->mapMySeedNodes.erase(sAddress);
        walletdb.EraseSeedNodeConfig(c.sAddress);
...
void MasternodeManager::on_startAllButton_clicked()
{
    ...
    for (PAIRTYPE(std::string, CSeedNodeConfig) adrenaline : pwalletMain->mapMySeedNodes)
    {
        CSeedNodeConfig c = adrenaline.second;
	std::string errorMessage;
        bool result = activeMasternode.RegisterByPubKey(c.sAddress, c.sMasternodePrivKey, c.sCollateralAddress, errorMessage);
	if(result)
	{
