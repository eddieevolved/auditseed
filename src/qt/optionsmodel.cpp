...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nAnonymizeSeedAmount"))
        settings.setValue("nAnonymizeSeedAmount", 1000);

    nDarksendRounds = settings.value("nDarksendRounds").toLongLong();
    nAnonymizeSeedAmount = settings.value("nAnonymizeSeedAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeSeedAmount"))
        SoftSetArg("-anonymizeseedamount", settings.value("nAnonymizeSeedAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in seed.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeSeedAmount:
            return QVariant(nAnonymizeSeedAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case DarkSendRounds:
            ...
            break;
        case AnonymizeSeedAmount:
            nAnonymizeSeedAmount = value.toInt();
            settings.setValue("nAnonymizeSeedAmount", nAnonymizeSeedAmount);
            Q_EMIT anonymizeSeedAmountChanged(nAnonymizeSeedAmount);
            break;
        case CoinControlFeatures:
