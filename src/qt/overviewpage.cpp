...
public:
    TxViewDelegate(const PlatformStyle *platformStyle):
            QAbstractItemDelegate(), unit(BitcoinUnits::SEED),
                                              ...
                                              filter(0)
{
    ...
    ui->setupUi(this);
    //set Logo
    ui->labelLogo->setPixmap(QPixmap(":/images/seed_logo_horizontal")
...
#else
       {
        if (fMasterNode) {
            ...
            if (!fEnableDarksend) {
                ...
                ui->toggleDarksend->setText(tr("Start Seedsend"));
            } else {
                ui->darksendEnabled->setText(tr("Enabled"));
                ui->toggleDarksend->setText(tr("Stop Seedsend"));
            }
...
void OverviewPage::updateDarkSendProgress()
{
    ...
    QString strAnonymizeSeedAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeSeedAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if (!fEnableDarksend) {
        ...
        strAnonymizeSeedAmount = strAnonymizeSeedAmount.remove(strAnonymizeSeedAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeSeedAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    ...
    if (nMaxToAnonymize > nAnonymizeSeedAmount * COIN) nMaxToAnonymize = nAnonymizeSeedAmount * COIN;

    if (nMaxToAnonymize == 0) return;

    if (nMaxToAnonymize >= nAnonymizeSeedAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                              .arg(strAnonymizeSeedAmount));
        strAnonymizeSeedAmount = strAnonymizeSeedAmount.remove(strAnonymizeSeedAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeSeedAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    } else {
...
void OverviewPage::darkSendStatus()
{
    ...
    if (strStatus != ui->darkSendStatus->text() && !strStatus.isEmpty()) {
        LogPrintf("Last Seedsend message: %s\n", strStatus.toStdString().c_str());
