...
bool ToHash160(const std::string& strSeedAddress, std::string& strHash160)
{   
    CTxDestination seedAddress = DecodeDestination(strSeedAddress);
    if(IsValidDestination(seedAddress)){     
        CKeyID *keyid = boost::get<CKeyID>(&seedAddress);
...
bool ToSeedAddress(const std::string& strHash160, std::string& strSeedAddress)
{
    uint160 key(ParseHex(strHash160.c_str()));
    CKeyID keyid(key);
    
    if(IsValidDestination(CTxDestination(keyid))){
        strSeedAddress = EncodeDestination(CTxDestination(keyid));
        return true;
    }
    return false;
}
...
bool Token::execEvents(int64_t fromBlock, int64_t toBlock, int func, std::vector<TokenEvent> &tokenEvents)
{
    ...
    for(int i = 0; i < list.size(); i++)
    {
        ...
        for(int i = 0; i < listLog.size(); i++)
        {
            ...
            ToSeedAddress(tokenEvent.sender, tokenEvent.sender);
            tokenEvent.receiver = topicsList[2].toString().toStdString().substr(24);
            ToSeedAddress(tokenEvent.receiver, tokenEvent.receiver);
