...
std::unique_ptr<CBlockTemplate> BlockAssembler::CreateNewBlock(const CScript& scriptPubKeyIn, bool fMineWitnessTx, bool fProofOfStake, int64_t* pTotalFees, int32_t txProofTime, int32_t nTimeLimit)
{
    ...
    globalSealEngine->setSeedSchedule(seedDGP.getGasSchedule(nHeight));
    uint32_t blockSizeDGP = seedDGP.getBlockSize(nHeight);
    minGasPrice = seedDGP.getMinGasPrice(nHeight);
    ...
    hardBlockGasLimit = seedDGP.getBlockGasLimit(nHeight);
...
bool BlockAssembler::AttemptToAddContractToBlock(CTxMemPool::txiter iter, uint64_t minGasPrice) {
    ...
    SeedTxConverter convert(iter->GetTx(), NULL, &pblock->vtx);

    ExtractSeedTX resultConverter;
    if(!convert.extractionSeedTransactions(resultConverter)){
    ...
    for(SeedTransaction seedTransaction : seedTransactions){
        txGas += seedTransaction.gas();
        if(txGas > txGasLimit) {
            // Limit the tx gas limit by the soft limit if such a limit has been specified.
            return false;
        }

        if(bceResult.usedGas + seedTransaction.gas() > softBlockGasLimit){
            //if this transaction's gasLimit could cause block gas limit to be exceeded, then don't add it
            return false;
        }
        if(seedTransaction.gasPrice() < minGasPrice){
    ...
    ByteCodeExec exec(*pblock, seedTransactions, hardBlockGasLimit);
...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet)
{
    // Found a solution (stake)
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("SEEDMiner : generated block is stale");

        for(const CTxIn& vin : pblock->vtx[1].vin) {
            if (wallet.IsSpent(vin.prevout.hash, vin.prevout.n)) {
                return error("SEEDMiner : Gen block stake is invalid - UTXO spent");
            }
        }
    }
    ...
    if (!ProcessNewBlock(state, chainparams, NULL, pblock)) {
        return error("SEEDMiner : ProcessNewBlock, block not accepted");
    }
