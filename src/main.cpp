...
#if defined(NDEBUG)
#error "SEED cannot be compiled without assertions."
#endif
...
std::unique_ptr<SeedState> globalState = nullptr;
...
unsigned int dgpMaxBlockSize = 2000000; // seed
...
const string strMessageMagic = "Seed Signed Message:\n";
...
#                   endif
        ...
        if(chainActive.Height() >= chainparams.FirstSCBlock() && tx.HasCreateOrCall()) {
            ...
            SeedDGP seedDGP(globalState.get(), fGettingValuesDGP);
            uint64_t minGasPrice = seedDGP.getMinGasPrice(chainActive.Height() + 1);
            uint64_t blockGasLimit = seedDGP.getBlockGasLimit(chainActive.Height() + 1);
            size_t count = 0;
            for(const CTxOut& o : tx.vout)
                count += o.scriptPubKey.HasOpCreate() || o.scriptPubKey.HasOpCall() ? 1 : 0;
            SeedTxConverter converter(tx, NULL);
            ExtractSeedTX resultConverter;
            if(!converter.extractionSeedTransactions(resultConverter)){
            ...
            std::vector<SeedTransaction> seedTransactions = resultConverter.first;
            std::vector<EthTransactionParams> seedETP = resultConverter.second;

            dev::u256 sumGas = dev::u256(0);
            dev::u256 gasAllTxs = dev::u256(0);
            for(SeedTransaction seedTransaction : seedTransactions){
                sumGas += seedTransaction.gas() * seedTransaction.gasPrice();
                ...
                if(txMinGasPrice != 0) {
                    txMinGasPrice = std::min(txMinGasPrice, seedTransaction.gasPrice());
                } else {
                    txMinGasPrice = seedTransaction.gasPrice();
                }
                VersionVM v = seedTransaction.getVersion();
                if(v.format!=0)
                ...
                if(seedTransaction.gas() < GetArg("-minmempoolgaslimit", MEMPOOL_MIN_GAS_LIMIT))
                ...
                if(seedTransaction.gas() < MINIMUM_GAS_LIMIT && v.rootVM != 0)
                ...
                if(seedTransaction.gas() > UINT32_MAX)
                ...
                if(v.rootVM!=0 && (uint64_t)seedTransaction.gasPrice() < minGasPrice)
            ...
            if(!CheckMinGasPrice(seedETP, minGasPrice))
                return state.DoS(100, false, REJECT_INVALID, "bad-txns-small-gasprice");

            if(count > seedTransactions.size())
...
CAmount GetMasternodePosReward(int nHeight, CAmount blockValue)
{
    ...
    if (nHeight >= POS_REWARD_CHANGED_BLOCK || IsTestNet()) {
        ...
        ret = blockValue * 0.8095; //40% for masternode
    }
    return ret;
}
...
#if 0
    SeedDGP seedDGP(globalState.get(), fGettingValuesDGP);
    globalSealEngine->setSeedSchedule(seedDGP.getGasSchedule(pindex->nHeight + 1));
#endif
    uint64_t minGasPrice = 0;//seedDGP.getMinGasPrice(pindex->nHeight + 1);
    uint64_t blockGasLimit = DEFAULT_BLOCK_GAS_LIMIT_DGP;//seedDGP.getBlockGasLimit(pindex->nHeight + 1);

#if 0
    uint32_t sizeBlockDGP = 0;//seedDGP.getBlockSize(pindex->nHeight + 1);
        ...
        if (fAddressIndex) { // OUTPUTS
            for (size_t k = 0; k < tx.vout.size(); k++) {
                ...
                if (ExtractDestination(out.scriptPubKey, dest, &txType)) {
                    ...
                    // outputs like OP_RETURN custom data, some SEED v3 vouts had 1 satoshi in nValue
...
///////////////////////////////////////////////////////////////////////////////////////// seed
        ...
        if (pindex->nHeight >= Params().FirstSCBlock()) {
            ...
            if(tx.HasCreateOrCall() && !hasOpSpend) {
                ...
                SeedTxConverter convert(tx, &view, &block.vtx);

                ExtractSeedTX resultConvertSeedTX;
                if(!convert.extractionSeedTransactions(resultConvertSeedTX)) {
                ...
                if(!CheckMinGasPrice(resultConvertSeedTX.second, minGasPrice))
                ...
                dev::u256 gasAllTxs = dev::u256(0);
                ByteCodeExec exec(block, resultConvertSeedTX.first, blockGasLimit);
                ...
                for(SeedTransaction& ltx : resultConvertSeedTX.first) {
                ...
                {
                    for(size_t k = 0; k < resultConvertSeedTX.first.size(); k ++){
                        ...
                        tri.push_back(TransactionReceiptInfo{block.GetHash(pindex->nHeight >= Params().SwitchPhi2Block()), uint32_t(pindex->nHeight), tx.GetHash(), uint32_t(i), resultConvertSeedTX.first[k].from(), resultConvertSeedTX.first[k].to(),
...
bool CheckForMasternodePayment(const CTransaction& tx, const CBlockHeader& header) {
    ...
    // Divide to keep a check precision of 0.01 SEED
...
bool ProcessNewBlock(CValidationState& state, const CChainParams& chainparams, CNode* pfrom, const CBlock* pblock, CDiskBlockPos* dbp)
{
    ...
    if (nHeight >= nSeedProtocolSwitchHeight)
    {
 ...
 * As an example, Seedcore uses /Seedcore:X.X.X/ version string. So, this function will
 * return 5 for /Seedcore:5.0.0/ or 0 if it couldn't find ":" or "." delimeters, or if no digits are between
 * those delimeters, or if string verison is empty
 * @param cleanVersion Seedcore wallet version as a string
 * @return Major Seedcore major version or 0
 */
...
std::vector<ResultExecute> CallContract(const dev::Address& addrContract, std::vector<unsigned char> opcode, const dev::Address& sender, uint64_t gasLimit){
    ...
    SeedTransaction callTransaction(0, 1, dev::u256(gasLimit), addrContract, opcode, dev::u256(0));
    callTransaction.forceSender(senderAddress);
    callTransaction.setVersion(VersionVM::GetEVMDefault());


    ByteCodeExec exec(block, std::vector<SeedTransaction>(1, callTransaction), blockGasLimit);
...
void writeVMlog(const std::vector<ResultExecute>& res, const CTransaction& tx, const CBlock& block){
    boost::filesystem::path seedDir = GetDataDir() / "vmExecLogs.json";
    std::stringstream ss;
    if(fIsVMlogFile){
        ss << ",";
    } else {
        std::ofstream file(seedDir.string(), std::ios::out | std::ios::app);
        file << "{\"logs\":[]}";
        file.close();
    }
    ...
    std::ofstream file(seedDir.string(), std::ios::in | std::ios::out);
...
bool SeedTxConverter::extractionSeedTransactions(ExtractSeedTX& seedtx){
    std::vector<SeedTransaction> resultTX;
    ...
    seedtx = std::make_pair(resultTX, resultETP);
    return true;
}

bool SeedTxConverter::receiveStack(const CScript& scriptPubKey){
...
bool SeedTxConverter::parseEthTXParams(EthTransactionParams& params){
    try{
...
SeedTransaction SeedTxConverter::createEthTX(const EthTransactionParams& etp, uint32_t nOut){
    SeedTransaction txEth;
    if (etp.receiveAddress == dev::Address() && opcode != OP_CALL){
        txEth = SeedTransaction(txBit.vout[nOut].nValue, etp.gasPrice, etp.gasLimit, etp.code, dev::u256(0));
    }
    else{
        txEth = SeedTransaction(txBit.vout[nOut].nValue, etp.gasPrice, etp.gasLimit, etp.receiveAddress, etp.code, dev::u256(0));
    }
