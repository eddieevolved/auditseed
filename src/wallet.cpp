...
void CWallet::AvailableCoinsMN(vector<COutput>& vCoins, bool fOnlyConfirmed, const CCoinControl *coinControl, AvailableCoinsType coin_type, bool useIX) const
{
...

    {
        ...
        for (map<uint256, CWalletTx>::const_iterator it = mapWallet.begin(); it != mapWallet.end(); ++it) {
            ...
            if (nDepth <= 0) // SeedNOTE: coincontrol fix / ignore 0 confirm
                continue;
            ...
            while (true) {
                ...
                if (!SelectCoins(wtxNew.strFromAccount, nTotalValue, setCoins, nValueIn, coinControl, coin_type, useIX)) {
                    if (coin_type == ALL_COINS) {
                        ...
                        strFailReason = _("Unable to locate enough DarkSend non-denominated funds for this transaction that are not equal 10000 SEED.");
                    } else {
...
#               endif
                ...
                if (nChange > 0) {
                    ...
                    // change transaction isn't always pay-to-seed-address
                    CScript scriptChange;
...
bool CWallet::AddSeedNodeConfig(CSeedNodeConfig nodeConfig)
{
    bool rv = CWalletDB(strWalletFile).WriteSeedNodeConfig(nodeConfig.sAlias, nodeConfig);
    if(rv)
	uiInterface.NotifySeedNodeChanged(nodeConfig);
