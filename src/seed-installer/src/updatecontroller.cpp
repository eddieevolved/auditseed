...
#include "seedupdater/seedupdater_p.h"

std::atomic<bool> running(false);
std::atomic<bool> wasCanceled(false);
QPointer<QWidget> win;
QtSeedUpdater::UpdateController::DisplayLevel gDisplayLevel;
QtSeedUpdater::ProgressDialog *gUpdatesProgress;
...
SeedUpdater *UpdateController::updater() const
{
	...
	if(!d->mainUpdater->checkForUpdates()) {
		...
		if(gDisplayLevel >= ExtendedInfoLevel) {
			...
			if(gDisplayLevel >= ProgressLevel) {
				...
				d->checkUpdatesProgress->open(d->mainUpdater, &QtSeedUpdater::SeedUpdater::abortUpdateCheck);
			}
		}
		return true;
	}
...
void UpdateController::checkUpdatesDone(bool hasUpdates, bool hasError)
{
	...
	if(wasCanceled) {
		...
		if(hasUpdates) {
			...
			if(hasError) {
				qCWarning(logSeedUpdater) << "maintenancetool process finished with exit code"
...
{
	return QIcon::fromTheme(QStringLiteral("system-software-update"), QIcon(QStringLiteral(":/res/icons/seedupdate.ico")));
}
...
UpdateControllerPrivate::UpdateControllerPrivate(UpdateController *q_ptr, const QString &version, QWidget *window) :
	q(q_ptr),
	window(window),
	mainUpdater(version.isEmpty() ? new SeedUpdater(q_ptr) : new SeedUpdater(version, q_ptr)),
	runAdmin(true),
	adminUserEdit(true),
	...
	scheduler(new SimpleScheduler(q_ptr))
{
	...
	QObject::connect(mainUpdater, &SeedUpdater::checkUpdatesDone,
...
{
	if(running)
		qCWarning(logSeedUpdater) << "UpdaterController destroyed while still running! This can crash your application!";
