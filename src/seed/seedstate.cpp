...
SeedState::SeedState(u256 const& _accountStartNonce, OverlayDB const& _db, const string& _path, BaseState _bs) :
        State(_accountStartNonce, _db, _bs) {
            dbUTXO = SeedState::openDB(_path + "/seedDB", sha3(rlp("")), WithExisting::Trust);
	        stateUTXO = SecureTrieDB<Address, OverlayDB>(&dbUTXO);
}

SeedState::SeedState() : dev::eth::State(dev::Invalid256, dev::OverlayDB(), dev::eth::BaseState::PreExisting) {
    dbUTXO = OverlayDB();
    stateUTXO = SecureTrieDB<Address, OverlayDB>(&dbUTXO);
}

ResultExecute SeedState::execute(EnvInfo const& _envInfo, SealEngineFace const& _sealEngine, SeedTransaction const& _t, Permanence _p, OnOpFunc const& _onOp){

    assert(_t.getVersion().toRaw() == VersionVM::GetEVMDefault().toRaw());

    addBalance(_t.sender(), _t.value() + (_t.gas() * _t.gasPrice()));
    newAddress = _t.isCreation() ? createSeedAddress(_t.getHashWith(), _t.getNVout()) : dev::Address();
    ...
    try{
        ...
        if (_p == Permanence::Reverted){
            ...
            seed::commit(cacheUTXO, stateUTXO, m_cache);
            cacheUTXO.clear();
...
std::unordered_map<dev::Address, Vin> SeedState::vins() const // temp
{
...
void SeedState::transferBalance(dev::Address const& _from, dev::Address const& _to, dev::u256 const& _value) {
...
Vin const* SeedState::vin(dev::Address const& _a) const
{
    return const_cast<SeedState*>(this)->vin(_a);
}

Vin* SeedState::vin(dev::Address const& _addr)
{
...
// void SeedState::commit(CommitBehaviour _commitBehaviour)
// {
...
void SeedState::kill(dev::Address _addr)
{
...
void SeedState::addBalance(dev::Address const& _id, dev::u256 const& _amount)
{
...
const dev::Address SeedState::createSeedAddress(dev::h256 hashTx, uint32_t voutNumber){
...
void SeedState::deleteAccounts(std::set<dev::Address>& addrs){
...
void SeedState::updateUTXO(const std::unordered_map<dev::Address, Vin>& vins){
    for(auto& v : vins){
...
void SeedState::printfErrorLog(const dev::eth::TransactionException er){
    std::stringstream ss;
    ss << er;
