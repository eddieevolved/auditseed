...
bool SeedDGP::checkLimitSchedule(const std::vector<uint32_t>& defaultData, const std::vector<uint32_t>& checkData){
...
dev::eth::EVMSchedule SeedDGP::getGasSchedule(unsigned int blockHeight){
    clear();
...
uint64_t SeedDGP::getUint64FromDGP(unsigned int blockHeight, const dev::Address& contract, std::vector<unsigned char> data){
    uint64_t value = 0;
...
uint32_t SeedDGP::getBlockSize(unsigned int blockHeight){
    clear();
...
uint64_t SeedDGP::getMinGasPrice(unsigned int blockHeight){
    clear();
...
uint64_t SeedDGP::getBlockGasLimit(unsigned int blockHeight){
    clear();
...
bool SeedDGP::initStorages(const dev::Address& addr, unsigned int blockHeight, std::vector<unsigned char> data){
    initStorageDGP(addr);
...
void SeedDGP::initStorageDGP(const dev::Address& addr){
    storageDGP = state->storage(addr);
}

void SeedDGP::initStorageTemplate(const dev::Address& addr){
    storageTemplate = state->storage(addr);
}

void SeedDGP::initDataTemplate(const dev::Address& addr, std::vector<unsigned char>& data){
    dataTemplate = CallContract(addr, data)[0].execRes.output;
}

void SeedDGP::createParamsInstance(){
...
dev::Address SeedDGP::getAddressForBlock(unsigned int blockHeight){
...
void SeedDGP::parseStorageScheduleContract(std::vector<uint32_t>& uint32Values){
...
void SeedDGP::parseDataScheduleContract(std::vector<uint32_t>& uint32Values){
...
void SeedDGP::parseStorageOneUint64(uint64_t& value){
...
void SeedDGP::parseDataOneUint64(uint64_t& value){
    if(dataTemplate.size() == 32){
        value = dev::toUint64(dev::u256(dev::h256(dataTemplate)));
    }
}

dev::eth::EVMSchedule SeedDGP::createEVMSchedule(){
    dev::eth::EVMSchedule schedule = dev::eth::EIP158Schedule;
