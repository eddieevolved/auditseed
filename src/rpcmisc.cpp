...
UniValue getinfo(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw runtime_error(
            ...
            "  \"darksend_balance\": xxxxxx, (numeric) the anonymized seed balance of the wallet\n"
            ...
            "  \"paytxfee\": x.xxxx,         (numeric) the transaction fee set in seed/kB\n"
            "  \"relayfee\": x.xxxx,         (numeric) minimum relay fee for non-free transactions in seed/kB\n"
...
UniValue getstateinfo(const UniValue& params, bool fHelp)
{
    ...
    SeedDGP seedDGP(globalState.get());

    UniValue obj(UniValue::VOBJ);
    obj.push_back(Pair("maxblocksize", (int)seedDGP.getBlockSize(chainActive.Height())));
    obj.push_back(Pair("blockgaslimit", (int)seedDGP.getMinGasPrice(chainActive.Height())));
    obj.push_back(Pair("mingasprice", (int)seedDGP.getBlockGasLimit(chainActive.Height())));
...
UniValue validateaddress(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            ...
            "\nReturn information about the given seed address.\n"
            "\nArguments:\n"
            "1. \"seedaddress\"     (string, required) The seed address to validate\n"
            "\nResult:\n"
            "{\n"
            "  \"isvalid\" : true|false,       (boolean) If the address is valid or not. If not, this is the only property returned.\n"
            "  \"address\" : \"seedaddress\",     (string) The seed address validated\n"
...
#ifdef ENABLE_WALLET
        // Case 1: SEED address and we have full public key:
...
UniValue createmultisig(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 2) {
        string msg = "createmultisig nrequired [\"key\",...]\n"
            ...
            "2. \"keys\"       (string, required) A json array of keys which are seed addresses or hex-encoded public keys\n"
            "     [\n"
            "       \"key\"    (string) seed address or hex-encoded public key\n"
            "       ,...\n"
            "     ]\n"
...
UniValue verifymessage(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 3)
        throw runtime_error(
            "verifymessage \"seedaddress\" \"signature\" \"message\"\n"
            "\nVerify a signed message\n"
            "\nArguments:\n"
            "1. \"seedaddress\"  (string, required) The seed address to use for the signature.\n"
...
UniValue getaddressbalance(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            ...
            "}\n"
            "\nResult: {\n"
            "  \"balance\",  (number) The current balance (in SEED satoshis)\n"
