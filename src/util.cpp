...
{
    assert(fileout == NULL);
    assert(fileoutVM == NULL); // seed
    assert(mutexDebugLog == NULL);

    boost::filesystem::path pathDebug = GetDataDir() / "debug.log";
    boost::filesystem::path pathDebugVM = GetDataDir() / "vm.log"; // seed
    fileout = fopen(pathDebug.string().c_str(), "a");
    fileoutVM = fopen(pathDebugVM.string().c_str(), "a"); // seed
    if (fileout) setbuf(fileout, NULL); // unbuffered
    if (fileoutVM) setbuf(fileoutVM, NULL); // unbuffered // seed
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL) {
        ...
        if (ptrCategory.get() == NULL) {
            ...
            // "seed" is a composite category enabling all SEED-related debug output
            if (ptrCategory->count(string("seed"))) {
...
// Windows < Vista: C:\Documents and Settings\Username\Application Data\SEED
// Windows >= Vista: C:\Users\Username\AppData\Roaming\SEED
// Mac: ~/Library/Application Support/SEED
// Unix: ~/.seed
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "SEED";
#else
    fs::path pathRet;
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "seed.conf"));
    ...
    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()) {
        // Create empty seed.conf if it does not exist
    ...
    for (boost::program_options::detail::config_file_iterator it(streamConfig, setOptions), end; it != end; ++it) {
        // Don't overwrite existing settings so command line settings override seed.conf
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "seedd.pid"));
