...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-conf=<file>", strprintf(_("Specify configuration file (default: %s)"), "seed.conf"));
...
#ifndef WIN32
    strUsage += HelpMessageOpt("-pid=<file>", strprintf(_("Specify pid file (default: %s)"), "seedd.pid"));
#endif
    ...
    strUsage += HelpMessageOpt("-port=<port>", strprintf(_("Listen for connections on <port> (default: %u or testnet: %u)"), 42420, 28333));
    ...
    strUsage += HelpMessageOpt("-mintxfee=<amt>", strprintf(_("Fees (in SEED/Kb) smaller than this are considered zero fee for transaction creation (default: %s)"), FormatMoney(CWallet::minTxFee.GetFeePerK())));
    ...
    strUsage += HelpMessageOpt("-anonymizeseedamount=<n>", strprintf(_("Keep N SEED anonymized (default: %u)"), 0));
    ...
    strUsage += HelpMessageOpt("-rpcport=<port>", strprintf(_("Listen for JSON-RPC connections on <port> (default: %u or testnet: %u)"), 42421, 9777));
...
std::string LicenseInfo()
{
    return FormatParagraph(strprintf(_("Copyright (C) 2009-%i The Bitcoin Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           ...
           "\n" +
           FormatParagraph(strprintf(_("Copyright (C) 2017-%i The SEED-Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           "\n" +
...
/** Sanity checks
 *  Ensure that SEED is running in a usable environment with all
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. Seedcore is shutting down."));
...
#endif
    // Make sure only a single SEED process is using the data directory.
    boost::filesystem::path pathLockFile = GetDataDir() / ".lock";
    FILE* file = fopen(pathLockFile.string().c_str(), "a"); // empty lock file; created if it doesn't exist.
    if (file) fclose(file);
    static boost::interprocess::file_lock lock(pathLockFile.string().c_str());
    ...
    if (!lock.timed_lock(boost::get_system_time() + boost::posix_time::seconds(10)))
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. Seedcore is probably already running."), strDataDir));
    ...
    while (!fLoaded && !fRequestShutdown) {
        ...
        do {
            try {
                ...
                boost::filesystem::path seedStateDir = GetDataDir() / "stateSeed";

                bool fStatus = boost::filesystem::exists(seedStateDir);
                const std::string dirSeed(seedStateDir.string());
                const dev::h256 hashDB(dev::sha3(dev::rlp("")));
                dev::eth::BaseState existsSeedState = fStatus ? dev::eth::BaseState::PreExisting : dev::eth::BaseState::Empty;
                globalState = std::unique_ptr<SeedState>(new SeedState(dev::u256(0), SeedState::openDB(dirSeed, hashDB, dev::WithExisting::Trust), dirSeed, existsSeedState));
                dev::eth::ChainParams cp((dev::eth::genesisInfo(dev::eth::Network::seedMainNetwork)));
                globalSealEngine = std::unique_ptr<dev::eth::SealEngineFace>(cp.createSealEngine());

                pstorageresult = new StorageResults(seedStateDir.string());
                if (fReset) {
...
// ********************************************************* Step 8: load wallet
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK) {
            if (nLoadWalletRet == DB_CORRUPT)
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of Seedcore") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE) {
                strErrors << _("Wallet needed to be rewritten: restart Seedcore to complete") << "\n";
    ...
    nAnonymizeSeedAmount = GetArg("-anonymizeSeedamount", 0);
    if (nAnonymizeSeedAmount > 999999) nAnonymizeSeedAmount = 999999;
    if (nAnonymizeSeedAmount < 2) nAnonymizeSeedAmount = 2;

    LogPrintf("SEED darksend rounds %d\n", nDarksendRounds);
    LogPrintf("Anonymize Seed Amount %d\n", nAnonymizeSeedAmount);
