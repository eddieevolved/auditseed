...
// Copyright (c) 2015-2017 The SEED developers
...
UniValue getnewaddress(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() > 2)
        throw runtime_error(
            ...
            "\nReturns a new SEED address for receiving payments.\n"
            ...
            "\nResult:\n"
            "\"seedaddress\"    (string) The new seed address\n"
            "\nExamples:\n" +
...
UniValue getaccountaddress(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            ...
            "\nReturns the current SEED address for receiving payments to this account.\n"
            "\nArguments:\n"
            "1. \"account\"       (string, required) The account name for the address. It can also be set to the empty string \"\" to represent the default account. The account does not need to exist, it will be created and a new address created  if there is no account by the given name.\n"
            "\nResult:\n"
            "\"seedaddress\"   (string) The account seed address\n"
            "\nExamples:\n" +
...
UniValue getrawchangeaddress(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() > 1)
        throw runtime_error(
            ...
            "\nReturns a new SEED address, for receiving change.\n"
...
UniValue setaccount(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw runtime_error(
            "setaccount \"seedaddress\" \"account\"\n"
            "\nSets the account associated with the given address.\n"
            "\nArguments:\n"
            "1. \"seedaddress\"  (string, required) The seed address to be associated with an account.\n"
...
UniValue getaccount(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            "getaccount \"seedaddress\"\n"
            "\nReturns the account associated with the given address.\n"
            "\nArguments:\n"
            "1. \"seedaddress\"  (string, required) The seed address for account lookup.\n"
            "\nResult:\n"
...
UniValue getaddressesbyaccount(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            ...
            "  \"seedaddress\"  (string) a seed address associated with the given account\n"
            "  ,...\n"
            "]\n"
            "\nExamples:\n" +
...
UniValue sendtoaddress(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 4)
        throw runtime_error(
            "sendtoaddress \"seedaddress\" amount ( \"comment\" \"comment-to\" )\n"
            "\nSend an amount to a given address. The amount is a real and is rounded to the nearest 0.00000001\n" +
            HelpRequiringPassphrase() +
            "\nArguments:\n"
            "1. \"seedaddress\"  (string, required) The seed address to send to.\n"
...
UniValue sendtoaddressix(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 4)
        throw runtime_error(
            "sendtoaddressix \"seedaddress\" amount ( \"comment\" \"comment-to\" )\n"
            "\nSend an amount to a given address. The amount is a real and is rounded to the nearest 0.00000001\n" +
            HelpRequiringPassphrase() +
            "\nArguments:\n"
            "1. \"seedaddress\"  (string, required) The seed address to send to.\n"
...
UniValue listaddressgroupings(const UniValue& params, bool fHelp)
{
    if (fHelp)
        throw runtime_error(
            ...
            "  [\n"
            "    [\n"
            "      \"seedaddress\",     (string) The seed address\n"
...
UniValue listaddressbalances(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() > 1)
        throw std::runtime_error(
            ...
            "\nResult:\n"
            "{\n"
            "  \"address\": amount, (string) Seed address and the amount in " + CURRENCY_UNIT + "\n"
            "  ,...\n"
            "}\n"
            "\nExamples:\n"
...
UniValue signmessage(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 2)
        throw runtime_error(
            "signmessage \"seedaddress\" \"message\"\n"
            "\nSign a message with the private key of an address" +
            HelpRequiringPassphrase() + "\n"
            "\nArguments:\n"
            "1. \"seedaddress\"  (string, required) The seed address to use for the private key.\n"
...
UniValue getreceivedbyaddress(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw runtime_error(
            "getreceivedbyaddress \"seedaddress\" ( minconf )\n"
            "\nReturns the total amount received by the given seedaddress in transactions with at least minconf confirmations.\n"
            "\nArguments:\n"
            "1. \"seedaddress\"  (string, required) The seed address for transactions.\n"
...
UniValue sendfrom(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 3 || params.size() > 6)
        throw runtime_error(
            "sendfrom \"fromaccount\" \"toseedaddress\" amount ( minconf \"comment\" \"comment-to\" )\n"
            "\nSent an amount from an account to a seed address.\n"
            "The amount is a real and is rounded to the nearest 0.00000001." +
            HelpRequiringPassphrase() + "\n"
            "\nArguments:\n"
            "1. \"fromaccount\"       (string, required) The name of the account to send funds from. May be the default account using \"\".\n"
            "2. \"toseedaddress\"  (string, required) The seed address to send funds to.\n"
...
UniValue sendmany(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 4)
        throw runtime_error(
            ...
            "    {\n"
            "      \"address\":amount   (numeric) The seed address is the key, the numeric amount in btc is the value\n"
            "      ,...\n"
            "    }\n"
    ...
    for (const string& name_ : keys) {
        ...
        if (!IsValidDestination(dest))
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, string("Invalid SEED address: ")+name_);
...
UniValue addmultisigaddress(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 3) {
        string msg = "addmultisigaddress nrequired [\"key\",...] ( \"account\" )\n"
            ...
            "Each key is a SEED address or hex-encoded public key.\n"
            "If 'account' is specified, assign address to that account.\n"

            "\nArguments:\n"
            "1. nrequired        (numeric, required) The number of required signatures out of the n keys or addresses.\n"
            "2. \"keysobject\"   (string, required) A json array of seed addresses or hex-encoded public keys\n"
            "     [\n"
            "       \"address\"  (string) seed address or hex-encoded public key\n"
            "       ...,\n"
            "     ]\n"
...
UniValue listtransactions(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() > 4)
        throw runtime_error(
            ...
            "    \"address\":\"seedaddress\",    (string) The seed address of the transaction. Not present for \n"
...
UniValue listsinceblock(const UniValue& params, bool fHelp)
{
    if (fHelp)
        throw runtime_error(
            ...
            "    \"address\":\"seedaddress\",    (string) The seed address of the transaction. Not present for move transactions (category = move).\n"
...
UniValue gettransaction(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw runtime_error(
            ...
            "      \"address\" : \"seedaddress\",   (string) The seed address involved in the transaction\n"
...
UniValue encryptwallet(const UniValue& params, bool fHelp)
{
    if (!pwalletMain->IsCrypted() && (fHelp || params.size() != 1))
        throw runtime_error(
            ...
            "\nNow we can so something like sign\n" + HelpExampleCli("signmessage", "\"seedaddress\" \"test message\"") +
    ...
    StartShutdown();
    return "wallet encrypted; seed server stopping, restart to run with encrypted wallet. The keypool has been flushed, you need to make a new backup.";
}
...
UniValue autocombinerewards(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 1)
        throw runtime_error(
            ...
            "Wallet will automatically monitor for any coins with value below the threshold amount, and combine them if they reside with the same SEED address\n"
...
UniValue multisend(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() != 2)
        throw runtime_error(
            ...
            "multisend <SEED Address> <percent>\n"
...
UniValue callcontract(const UniValue& params, bool fHelp)
{
    ...
    if(params.size() == 3){
        CTxDestination seedSenderAddress = DecodeDestination(params[2].get_str());
        if(IsValidDestination(seedSenderAddress)) {
            CKeyID *keyid = boost::get<CKeyID>(&seedSenderAddress);
...
UniValue createcontract(const UniValue& params, bool fHelp){
    ...
    uint64_t blockGasLimit = seedDGP.getBlockGasLimit(chainActive.Height());
    uint64_t minGasPrice = CAmount(seedDGP.getMinGasPrice(chainActive.Height()));
    ...
    if (fHelp || params.size() < 1 || params.size() > 6)
        throw runtime_error(
                ...
                "3. gasPrice  (numeric or string, optional) gasPrice SEED price per gas unit, default: "+FormatMoney(nGasPrice)+", min:"+FormatMoney(minGasPrice)+"\n"
    ...
    if (params.size() > 3){
        ...
        if (!IsValidDestination(senderAddress))
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid Seed address to send from");
        else
            fHasSender=true;
    }
...
UniValue sendtocontract(const UniValue& params, bool fHelp){
    ...
    if (fHelp || params.size() < 2 || params.size() > 8)
        throw runtime_error(
                ...
                "5. gasPrice  (numeric or string, optional) gasPrice Seed price per gas unit, default: "+FormatMoney(nGasPrice)+", min:"+FormatMoney(minGasPrice)+"\n"
