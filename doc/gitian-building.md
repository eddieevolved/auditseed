...
*Setup instructions for a gitian build of SEED using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the SEED
...
Clone the git repositories for seed and gitian and then checkout the seed version that you want to build.
...
Follow the instructions in [doc/release-process.md](release-process.md) in the seed repository
...
To build SEED (for Linux, OSX and Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the seed repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/seed/.git/
    remote: Reusing existing pack: 35606, done.
    remote: Total 35606 (delta 0), reused 0 (delta 0)
    Receiving objects: 100% (35606/35606), 26.52 MiB | 4.28 MiB/s, done.
    Resolving deltas: 100% (25724/25724), done.
    From https://github.com/devstar11/seed
...
For example:
```bash
URL=https://github.com/crowning-/seed.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit seed=${COMMIT} --url seed=${URL} ../seed/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit seed=${COMMIT} --url seed=${URL} ../seed/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit seed=${COMMIT} --url seed=${URL} ../seed/contrib/gitian-descriptors/gitian-osx.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/seed-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/seed-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/seed-build.assert
```
...
[seed/gitian.sigs](https://github.com/devstar11/gitian.sigs/) repository, or if that's not possible to create a pull
request.
